...
python3 -c 'import ast; [ast.parse(open(path).read()) for path in ("scripts/measure_similarity.py", "scripts/compare_offpolicy_arms.py", "scripts/measure_offpolicy_arms.py")]'
git diff --check
