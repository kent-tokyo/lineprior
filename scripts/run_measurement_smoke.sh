...
python3 "$root/scripts/measure_similarity.py" "$prior" "$root/examples/similarity_queries.jsonl" --out "$similarity_first" --dataset-id fixture-v1 --split heldout --feature-version fixture-v1 --lineprior-version 0.12.0
python3 "$root/scripts/measure_similarity.py" "$prior" "$root/examples/similarity_queries.jsonl" --out "$similarity_second" --dataset-id fixture-v1 --split heldout --feature-version fixture-v1 --lineprior-version 0.12.0
...
python3 "$root/scripts/measure_offpolicy_arms.py" "$root/examples/offpolicy_off.jsonl" "$root/examples/offpolicy_on.jsonl" --lineprior-bin "$binary" --out "$integrated" --dataset-id fixture-v1 --split heldout --policy-version 0.12.0 --bootstrap-resamples 64 --bootstrap-seed 42
python3 -c 'import json, pathlib, sys; r=json.loads(pathlib.Path(sys.argv[1]).read_text()); assert r["protocol"] == "offpolicy-integrated-arms-v1"; assert r["arms"]["off"]["doubly_robust"]["estimate"] is not None; assert r["measurement"]["input_sha256"] == r["paired"]["measurement"]["input_sha256"]; assert r["measurement"]["dataset_id"] == r["paired"]["measurement"]["dataset_id"]; assert r["measurement"]["split"] == r["paired"]["measurement"]["split"]; assert r["paired"]["paired_reward_delta_on_minus_off"] == 0.5' "$integrated"
