...
**既知の制約:** AGENTS.md の MVP パフォーマンス目標では、メモリ使用量がユニークな `(state, action)` ペア数に比例して「有界」であることを求めています。現在の実装はこれを満たしていません — `parse_jsonl` は集計前にすべての観測を Vec としてメモリ上に完全展開するため、ピークメモリはユニークペア数ではなく総観測数に比例してスケールします。これを修正するには `parse_jsonl` の公開APIを、即座に確定する `Vec` からストリーミングイテレータへ作り直し、すべての呼び出し元を更新する必要があります。これは本格的なAPI再設計であり、今回のスコープでは対応せず、フォローアップ課題として記録しています。

チェックイン済みの小規模なベンチマークは `crates/lineprior/benches/scoring.rs` にあります(`cargo bench -p lineprior` で実行)。`build_prior_book` を 1,000 / 10,000 / 50,000 件の観測規模でカバーし、日常的なリグレッション検知に使えます。
